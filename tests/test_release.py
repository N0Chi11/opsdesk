...
class ReleaseFixtureTests(unittest.TestCase):
    ...
    def test_archive_is_reproducible_and_metadata_is_normalized(self):
        ...
        # Windows 无 POSIX 权限位，os.chmod 只切换只读标志
        expected_zip_mode = 0o644 if os.name != "nt" else 0o666
        self.assertEqual(stat.S_IMODE(second.stat().st_mode), expected_zip_mode)
        with zipfile.ZipFile(second) as archive:
            infos = {info.filename: info for info in archive.infolist()}
        regular_info = infos["OpsDesk-1.2.3/server.py"]
        executable_info = infos["OpsDesk-1.2.3/start.command"]
