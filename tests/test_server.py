...
class ParsingTests(unittest.TestCase):
    ...
    def test_listener_scan_preserves_ipv6_loopback_for_open_links(self):
        if server.IS_WINDOWS:
            # Windows 走 netstat 解析（_win_scan_listeners）
            output = """TCP    127.0.0.1:8000    0.0.0.0:0              LISTENING       202
...
"""
            with mock.patch.object(server, "run_cmd", return_value=output):
                listeners = server._win_scan_listeners()
        else:
            ...
            with tempfile.TemporaryDirectory() as td:
                for suffix, runner in cases.items():
                    with self.subTest(suffix=suffix):
                        ...
                        self.assertEqual(parts[:len(runner)], runner)
                        self.assertTrue(parts[-1].endswith(path))
        else:
            cases = {
                ".py": "python3",
                ".zsh": "/bin/zsh",
...
class AppHealthTests(unittest.TestCase):
    ...
    def test_relative_script_uses_configured_working_directory(self):
        with tempfile.TemporaryDirectory() as td:
            if server.IS_WINDOWS:
                name, command = "job.py", "python -- job.py"
            else:
                name, command = "job.sh", "/bin/bash -- job.sh"
            with open(os.path.join(td, name), "w", encoding="utf-8") as handle:
                handle.write("print(1)\n" if name.endswith(".py") else "echo ok\n")
    ...
    def test_direct_script_requires_execute_permission_but_bash_script_does_not(self):
        if server.IS_WINDOWS:
            return  # Windows 无执行位语义，直接脚本按可执行处理
        with tempfile.TemporaryDirectory() as td:
    ...
    def test_task_cancel_exit_code_survives_shell_wrapper(self):
        with tempfile.TemporaryDirectory() as td, \
            ...
            app = {"id": "deadbeef", "cwd": td,
                   "command": (server.IS_WINDOWS and
                               "python -c \"raise SystemExit(130)\"" or
                               "python3 -c 'raise SystemExit(130)'")}
...
class ProjectDetectionTests(unittest.TestCase):
    ...
    def test_detects_django_and_static_site_fallback(self):
        ...
        python_cmd = "python" if server.IS_WINDOWS else "python3"
...
class ConfigTests(unittest.TestCase):
    ...
    def test_atomic_write_keeps_previous_good_version_as_backup(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            expected = "0o600" if not server.IS_WINDOWS else "0o666"
            self.assertEqual(oct(os.stat(path).st_mode & 0o777), expected)
...
class RuntimeStorageTests(unittest.TestCase):
    ...
    def test_first_run_copies_legacy_data_privately_without_deleting_source(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            if not server.IS_WINDOWS:  # Windows 无 POSIX 权限位
...
class ProcessIdentityTests(unittest.TestCase):
    ...
    def test_real_started_process_is_identified_and_stoppable(self):
        with tempfile.TemporaryDirectory() as td, \
                mock.patch.object(server, "LOGS_DIR", td):
            command = (server.IS_WINDOWS and
                       "python -c \"import time; time.sleep(20)\"" or "sleep 20")
    ...
    def test_verified_legacy_process_can_be_stopped_without_port_kill(self):
        ...
        with mock.patch.object(server, "managed_pids", return_value=[]), \
                ...
                mock.patch.object(server, "_win_taskkill",
                                  return_value=(True, None)) as stop_win, \
            ...
            self.assertTrue(stopped, error)
        if server.IS_WINDOWS:
            stop_win.assert_called_once_with(999)
        else:
...
class LaunchEnvironmentTests(unittest.TestCase):
    def test_headless_launch_path_includes_common_user_node_locations(self):
        ...
        if server.IS_WINDOWS:
            self.assertIn(os.environ.get("APPDATA", ""), env["PATH"])
        else:
...
class IconTests(unittest.TestCase):
    ...
    def test_external_favicon_links_are_rejected_and_real_svg_accepted(self):
        png = b"\x89PNG\r\n\x1a\n" + b"payload"
        ...
        # 真实 SVG 载荷（以 <svg / <?xml 开头）按契约支持；
        # HTML、脚本或其他内容仍拒绝（不能把任意页面当图标）。
        self.assertEqual(server.sniff_icon_bytes(
            b'<svg xmlns="http://www.w3.org/2000/svg"></svg>',
            "image/svg+xml"), "svg")
        self.assertIsNone(server.sniff_icon_bytes(
            b"<html><head></head></html>", "text/html"))
        self.assertIsNone(server.sniff_icon_bytes(
            b"<script>alert(1)</script><svg></svg>", "text/html"))
