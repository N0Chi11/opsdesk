...
- [ ] Git 工作区干净，没有未审查的改动。
- [ ] `make release-check` 通过，完整输出已归档（Windows 上用 `python tools/check_project.py --release` 等价命令）。
- [ ] 测试数量大于 0，失败与错误均为 0（macOS 与 Windows 双平台）。
- [ ] Python、JavaScript、Bash/批处理、VBS、plist 和主题 JSON 语法检查通过（Windows 检查 `start.bat`/`总控台.vbs`，跳过 bash/plutil）。
...
- [ ] 普通停止、忽略 SIGTERM（仅 macOS）、快速自然退出、启动失败和重启均已验收（Windows 验收普通停止与进程树结束）。
...
- [ ] 从上一发布版本的真实备份升级成功，应用、图标、标记和主题未丢失（Windows：`%APPDATA%\总控台` / `%LOCALAPPDATA%\总控台`；macOS：目录 0700、文件 0600）。
