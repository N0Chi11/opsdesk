...
def command_output(args: list[str], cwd: Path = ROOT) -> str:
    try:
        # Windows 控制台默认 GBK；node 输出 UTF-8，统一按 UTF-8 解码（替换损坏字节）。
        # Python 子进程输出由 PYTHONUTF8=1 保证为 UTF-8。
...
def check_shell_and_plist() -> str:
    if sys.platform == "win32":
        # Windows：无 bash/plutil；校验启动器存在且非空，plist 用 plistlib 解析。
        launchers = (ROOT / "start.bat", ROOT / "总控台.vbs")
        for path in launchers:
            require(path.is_file(), f"缺少 Windows 启动器: {path.name}")
            require(path.stat().st_size > 0, f"Windows 启动器为空: {path.name}")
        with INFO_PLIST.open("rb") as handle:
            plistlib.load(handle)  # 语法校验
        return "2 个 Windows 启动器 + Info.plist"
    shell_files = (
        ROOT / "start.command",
...
def check_javascript_tests() -> str:
    ...
    # node 24+ 用 "ℹ pass" 前缀，旧版用 "# pass"
    match = re.search(r"[#ℹ] (pass)\s+(\d+)", output)
    require(match is not None, "无法确认 node --test 结果")
    passed = int(match.group(2))
    require(not re.search(r"[#ℹ] fail\s+[1-9]\d*", output),
            "JavaScript 测试存在失败项")
    return f"{passed} 个测试"
