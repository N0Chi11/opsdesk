...
本地服务监控与快速启动控制台。**零依赖**：Python 3 标准库后端（单文件）+ 无构建原生前端。Windows 推荐双击 `总控台.vbs` 后台运行（无控制台窗口），`start.bat` 保留为终端调试入口；macOS 对应 `总控台.app` / `start.command`。
...
- Windows 用户数据：`%APPDATA%\总控台\config.json`（配置；`icons/` 为应用图标）、`%LOCALAPPDATA%\总控台\{appId}.log`（应用日志）与 `console.log`（启动器日志）；macOS 对应 `~/Library/Application Support/总控台/` 与 `~/Library/Logs/总控台/`（目录/文件权限 0700/0600）
- `data/` — 旧版项目内数据，仅在新目标不存在的首次启动中复制迁移；保留不删除
- `start.bat` — Windows 终端启动脚本；`总控台.vbs` — Windows 无窗口双击启动器（内部 `pythonw.exe server.py --launcher`，输出写入日志目录的 `console.log`）
...
`python server.py`（Windows）/ `python3 server.py`（macOS）→ 绑定 `127.0.0.1`，端口从 **9600** 起尝试，被占则 +1（最多 10 个）。启动后自动打开浏览器。`/favicon.ico` 返回统一品牌图标。双击启动器（`总控台.vbs` / `总控台.app`）会先识别同目录的现有总控台，可直接打开或安全重启，不需要用户输入命令，也不会出现终端窗口。
...
- **端口扫描**：macOS 用 `lsof -iTCP -sTCP:LISTEN -P -n`，Windows 用 `netstat -ano -p tcp`，均按 `(pid, port)` 去重（IPv4/6 重复行），保留绑定主机供 `listener_open_host` 判断。
- **进程详情**：macOS 批量 `ps -o pid=,user=,comm=,args=,%cpu=,%mem=,etime=`；Windows 用 NtQuerySystemInformation（pid/ppid/创建时间/CPU 计时）+ GetProcessMemoryInfo（工作集）+ PEB 读取（可执行路径/命令行/工作目录）+ 进程 Token SID（归属校验，`same_user` 统一判断），只保留当前用户的进程。
- **cwd**：macOS 用 `lsof -a -p <pids> -d cwd -Fn`；Windows 读 PEB 的 `CurrentDirectory`（x64 偏移 0x38），读不到则返回空。
...
- **关注进程**：macOS 用 `ps -axo ...`；Windows 复用同一套进程快照（NtQuery + PEB），args 小写包含关键字即命中，只保留当前用户并排除自身及系统扫描工具。
- **应用状态**：每次启动生成随机 `runToken`，常驻外层进程在 argv 中持有标记并等待内层命令：macOS 用 bash 双层脚本（进程组 pgid），Windows 用 `server.py --run-app` 包装进程（ppid 进程树，根 pid 即 lastPgid）。新版进程只有同时命中 `lastPgid` / 当前用户 / token 的进程组才算 running；升级前缺少 token 的旧进程，只有配置 `lastPid`、监听端口、当前用户与真实 cwd 全部一致时才兼容认领。用户明确从服务监控认领的 `attached` 卡片允许监听子进程换 PID，但必须在配置端口上按当前用户 + 真实 cwd 唯一命中；任一条件不符仍按外部端口占用处理。`ports` 来自受控进程组成员实际监听的端口。
- **应用启停**：多张卡片可保存相同端口（例如多个默认使用 3000 的项目）；启动前只拒绝失效配置和当时真实被占用的端口。重启先做健康预检，失败时不会先停掉仍工作的旧服务。停止时先校验 token，然后只对该受控进程组发信号（macOS `SIGTERM`；Windows `TerminateProcess` 结束整棵进程树），**绝不按端口杀其他监听者**。服务手动 stop 不记录退出历史；任务自然结束记录四态结果，总控台中止记录 `stopped`。批处理不做“长期服务存活探测”，避免把快速成功误判成失败
...
- **无终端 PATH**：Finder/`LSUIElement`（macOS）或资源管理器/pythonw（Windows）启动不会读取 shell 配置；子应用启动环境需显式补入常用工具目录（macOS：`~/.local/bin`、Volta/Bun/pnpm、NVM/fnm、Homebrew；Windows：`%APPDATA%\npm`、`%LOCALAPPDATA%\Microsoft\WinGet\Links`、scoop shims），保证 `node`/`npm`/`pnpm` 等可用。Windows 子进程环境还会设置 `PYTHONIOENCODING=utf-8` / `PYTHONUTF8=1` / `PYTHONUNBUFFERED=1`，保证日志文件按 UTF-8 实时写入。启动 API 短暂探测立即退出，并把日志末行作为明确错误返回。
...
- **运行目录**：Windows 默认配置/图标位于 `%APPDATA%\总控台`，日志位于 `%LOCALAPPDATA%\总控台`；macOS 位于 `~/Library/Application Support/总控台` 与 `~/Library/Logs/总控台`。`CONSOLE_DATA_DIR` / `CONSOLE_LOG_DIR` 可显式覆盖，覆盖时对应目录不自动迁移旧 `data/`。Windows 无 POSIX 权限位，目录/文件 `mode` 参数被忽略（显式 mode 反而会生成受限 DACL）。
