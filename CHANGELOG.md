...
- 项目命令识别现在会展示更多服务与批处理候选，并将 build/test/lint/check 等脚本正确归类为任务，避免构建脚本被误当成长期服务。
...
- **Windows 支持**：后端平台层完成 Windows 移植（`netstat` 监听扫描、
  ...
  数据目录、`python` 启动包装进程与 `--run-app` 运行身份）。macOS 原有实现完整保留，双平台共用同一代码库。
- 新增 Windows 启动器：`start.bat`（终端入口，等价 `start.command`）与 `总控台.vbs`
  （无窗口双击启动，等价 `总控台.app`）。
- 前端键位与外观适配 Windows：⌘ 键位自动显示为 Ctrl+、字体栈加入 Segoe UI、
  路径缩写支持 Windows 用户目录前缀形式。
- CI 改为 windows-latest + macos-15 双平台矩阵；`tools/check_project.py` 与发行包在 Windows 上可运行。
...
- 服务监控增加「扫描现有服务」批量导入：可一次选择多个当前用户的未管理监听服务，后端会重新核验并逐项报告失效/重复候选。
- 应用状态增加只读配置健康检查，在运行前识别丢失的工作目录、脚本和运行时，并提供修复入口。

### Changed

- 项目对外品牌更名为 OpsDesk，并新增上游来源、版权与衍生修改说明。
- 补充跨平台开源发布文档，明确 Windows / macOS 支持范围和第三方素材复核门禁。
- Windows 用户数据默认位置：配置/图标在 `%APPDATA%\总控台`，日志在 `%LOCALAPPDATA%\总控台`；macOS 仍为 `~/Library/Application Support/总控台` 与 `~/Library/Logs/总控台`。
- Windows 上 `os.chmod`/目录 `mode` 不再影响 ACL（Windows 无 POSIX 权限位）；健康检查跳过权限位检查。
- 进程停止语义：Windows 用 TerminateProcess 结束受控进程树（无 POSIX 信号）；批处理任务退出码约定不变（0 成功 / 130 取消 / 其余失败）。
...
- 修正 Windows 服务自动识别：系统目录、Program Files 和明确的桌面 GUI 后台进程不再被列为可批量导入的开发服务候选。
