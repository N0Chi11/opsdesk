...
import { $, el, setText, setChildren, icon, escapeHtml,
  ...
  localServiceUrl, keyLabel } from './js/core.js';
...
stopConsoleBtn.addEventListener('click', () => {
  ...
  openConfirm({
    ...
    onOk: async () => {
      ...
      banner.textContent = '总控台已停止。再次启动请双击项目目录中的启动器（Windows：start.bat 或 总控台.vbs）。';
    },
  });
});
...
if (!/Mac/i.test(navigator.platform || '')) {
  /* Windows 上把静态 kbd 标签从 ⌘K 显示为 Ctrl+K */
  document.querySelectorAll('kbd').forEach(k => {
    k.textContent = k.textContent.replace(/⌘/g, 'Ctrl+');
  });
}
