...
function fallbackScriptCommand(path) {
  ...
  if (suffix === '.py') return (isWin ? 'python -- ' : 'python3 -- ') + quoted;
  if (suffix === '.bat' || suffix === '.cmd') return 'cmd /c ' + quoted;
  if (suffix === '.ps1') return 'powershell -NoProfile -ExecutionPolicy Bypass -File ' + quoted;
  if (isWin) return quoted;
...
export function initAppModal({ onAddService, onAddTask }) {
  ...
  btnPickScript.addEventListener('click', async () => {
    ...
    try {
      const r = await act(post('/api/pick', { what: 'script' }, 300000));
      ...
      if (!fName.value.trim()) {
        const base = p.split(sep).pop().replace(/\.(command|sh|bash|zsh|py|bat|cmd|ps1|vbs)$/i, '');
  ...
  btnPickCwd.addEventListener('click', async () => {
    btnPickCwd.disabled = true;
    try {
      const r = await act(post('/api/pick', { what: 'dir' }, 300000));
