...
if IS_WINDOWS:
    # Windows 用户数据目录：%APPDATA%\总控台，日志：%LOCALAPPDATA%\总控台
    DEFAULT_DATA_DIR = os.path.expandvars(
        r"%APPDATA%\总控台")
    DEFAULT_LOGS_DIR = os.path.expandvars(
        r"%LOCALAPPDATA%\总控台")
else:
...
SELF_PID = os.getpid()
if IS_WINDOWS:
    # Windows 没有 UID 概念：快照/查询中当前用户的进程以 0 为哨兵标识，
    # 其他用户进程以 tasklist 返回的用户名字符串（或 -1 未知）标识。
    SELF_UID = 0
    _WIN_USERNAME = (os.environ.get("USERNAME") or "").casefold()
    _WIN_USERDOMAIN = (os.environ.get("USERDOMAIN") or "").casefold()

    def same_user(value):
        """Windows 归属判断：0 哨兵、SID 精确匹配，或用户名（可带域）匹配。"""
        if value == 0:
            return True
        ...
        if text.startswith("s-1-"):
            try:
                return text == str(_win_current_user_sid() or "").casefold()
            except Exception:
                return False
        if text in ("system", "local service", "network service"):
            return False
        return text.split("\\")[-1] == _WIN_USERNAME or text == _WIN_USERNAME
else:
    SELF_UID = os.getuid()

    def same_user(value):
        return value == SELF_UID

ICON_EXTS = (".png", ".jpg", ".jpeg", ".webp", ".ico", ".svg")
...
def _ensure_private_dir(path):
    ...
    if IS_WINDOWS:
        # Windows 无 POSIX 权限位：显式 mode 会让 CPython 生成不含继承 ACE
        # 的 DACL，在受限账户/沙箱下导致新建目录无法写入。
    ...
    if (not data_overridden and data_dir != legacy_data_dir
        ...
        def populate_data(staging):
            ...
            if os.path.isdir(source_icons) and not os.path.islink(source_icons):
                ...
                os.mkdir(target_icons, 0o777 if IS_WINDOWS else 0o700)
...
def _lock_exclusive(lock_file):
    ...
    if IS_WINDOWS:
        # msvcrt 只能锁文件里已有的字节；先确保有 1 字节可锁。
        # 注意：Windows 字节范围锁是强制的——别的句柄持锁时，本句柄对
        # 锁定区域的读写会直接 EACCES，因此把“探测占用”合并进这里的读写。
        try:
            lock_file.seek(0)
...
def _unlock_exclusive(lock_file):
    if IS_WINDOWS:
        try:
            lock_file.seek(0)
...
def acquire_instance_lock(path=INSTANCE_LOCK_PATH):
    ...
    if not _lock_exclusive(lock_file):
        lock_file.close()
        return None
    try:
...
def run_cmd(args, timeout=SUBPROCESS_TIMEOUT):
    ...
    try:
        ...
        return ""
# ---------------------------------------------------------------- Windows 平台层
# 全部基于 Python 标准库 + Windows 自带工具（netstat/tasklist），
# 不依赖 WMI / PowerShell / 第三方包；行为与 macOS 的 ps/lsof 分支等价。


def win_split_command(command):
    """按 cmd 规则拆分命令行为 argv：双引号分组，"" 为字面引号。

    只处理简单调用（无管道/重定向/变量展开）时用，可直接交给 CreateProcess，
    绕开 cmd /c 对引号的破坏性重解析。
    """
    tokens, cur, quoted = [], [], False
    index = 0
...
def _win_api():
    """惰性加载 Windows 原生 API，避免在非 Windows 上触碰。"""
    global _win_ntdll, _win_kernel32
    if _win_ntdll is None:
        import ctypes
    ...
    return _win_ntdll, _win_kernel32


def _win_u16(data, offset):
    return int.from_bytes(data[offset:offset + 2], 'little')


def _win_u32(data, offset):
    return int.from_bytes(data[offset:offset + 4], 'little')


def _win_u64(data, offset):
    return int.from_bytes(data[offset:offset + 8], 'little')


def _win_pid_offsets():
    """SYSTEM_PROCESS_INFORMATION 关键字段偏移（x64 / x86）。"""
    if sys.maxsize > 2**32:
        return {'create': 0x20, 'user': 0x28, 'kernel': 0x30,
                'name_len': 0x38, 'name_ptr': 0x40, 'pid': 0x50,
                'ppid': 0x58, 'ws': 0x78, 'fixed': 0x100}
    return {'create': 0x20, 'user': 0x28, 'kernel': 0x30,
            'name_len': 0x38, 'name_ptr': 0x3C, 'pid': 0x44,
            'ppid': 0x48, 'ws': 0x5C, 'fixed': 0x78}


def _win_filetime_to_unix(value):
    """FILETIME（1601 起 100ns）→ Unix 秒。"""
    return value / 10000000.0 - 11644473600.0


def _win_nt_query_processes():
    """NtQuerySystemInformation 全量进程表 → {pid: base}。

    返回每个进程的 ppid/名称/创建时间/CPU 计时/工作集；无需句柄，
    因此不受提权差异影响。
    """
    ntdll, _ = _win_api()
    import ctypes
    ...
    while size <= 16 * 1024 * 1024:
        ...
        rc = ntdll.NtQuerySystemInformation(
            5, buf, size, ctypes.byref(needed))
        if rc == 0:
            break
        # NTSTATUS 是 32 位有符号；0xC0000004 = STATUS_INFO_LENGTH_MISMATCH
        if (rc & 0xFFFFFFFF) != status_info_length_mismatch:
            LOG.warning('NtQuerySystemInformation 失败: 0x%08X', rc & 0xFFFFFFFF)
            return {}
        size = max(size * 2, int(needed.value) + 65536)
    else:
        return {}
    data = buf.raw
    offs = _win_pid_offsets()
    result = {}
    offset = 0
    total = len(data)
    while offset < total:
        next_off = _win_u32(data, offset)
        pid = _win_u64(data, offset + offs['pid'])
        ppid = _win_u64(data, offset + offs['ppid'])
        create = _win_u64(data, offset + offs['create'])
        user = _win_u64(data, offset + offs['user'])
        kernel = _win_u64(data, offset + offs['kernel'])
        ws = _win_u64(data, offset + offs['ws'])
        name_len = _win_u16(data, offset + offs['name_len'])
        name_ptr = _win_u64(data, offset + offs['name_ptr'])
        name = ''
        if name_len > 0 and name_len <= 4096 and name_ptr > 0x10000:
            try:
                name = ctypes.string_at(
                    ctypes.c_void_p(name_ptr), name_len
                ).decode('utf-16-le', 'replace').rstrip('\x00')
            except (OSError, ValueError):
                name = ''
        result[pid] = {
            'ppid': ppid,
            'name': name or 'unknown',
            'created': _win_filetime_to_unix(create),
            'ticks': (user + kernel) / 10000000.0,  # 秒
            'ws': ws,
        }
        if next_off == 0:
            break
...
_WIN_SNAPSHOT_CACHE = {'mono': 0.0, 'raw': None}
_WIN_SNAPSHOT_TTL = 2.0
_WIN_CPU_DELTA = {'mono': 0.0, 'ticks': {}, 'times': {}}
_WIN_MEM_TOTAL = {'mono': 0.0, 'bytes': 0}
_WIN_PEB_CACHE = {'mono': 0.0, 'items': {}}  # pid -> {exe, cmd, cwd}


def _win_total_memory():
    """物理内存总量（字节），30 秒缓存。"""
    now = time.monotonic()
    if now - _WIN_MEM_TOTAL['mono'] < 30.0 and _WIN_MEM_TOTAL['bytes']:
        return _WIN_MEM_TOTAL['bytes']
    import ctypes
    ...
    if kernel32.GlobalMemoryStatusEx(ctypes.byref(status)):
        _WIN_MEM_TOTAL['bytes'] = status.ullTotalPhys
        _WIN_MEM_TOTAL['mono'] = time.monotonic()
        return status.ullTotalPhys
    return 0


def _win_read_mem(handle, address, size):
    kernel32 = _win_api()[1]
    import ctypes
    buf = ctypes.create_string_buffer(size)
...
def _win_process_peb(pid, force=False):
    """读取进程 PEB：{exe, cmd, cwd}；读不到返回 {}。

    x64 偏移：ImagePathName 0x60 / CommandLine 0x70 / CurrentDirectory 0x38；
    x86 偏移：0x38 / 0x40 / 0x24（UNICODE_STRING）。
    """
    now = time.monotonic()
    cached = _WIN_PEB_CACHE['items'].get(pid)
    if cached is not None and not force and \
            now - _WIN_PEB_CACHE['mono'] < _WIN_SNAPSHOT_TTL:
        return cached
    kernel32 = _win_api()[1]
    ntdll = _win_api()[0]
    import ctypes
    ...
    handle = kernel32.OpenProcess(
        0x0400 | 0x0010, False, int(pid))  # QUERY_INFORMATION | VM_READ
    if not handle:
        return {}
    try:
        ...
        if ntdll.NtQueryInformationProcess(
                handle, 0, ctypes.byref(pbi), ctypes.sizeof(pbi),
                ctypes.byref(size)) != 0:
            return {}
        peb_data = _win_read_mem(handle, pbi.PebBaseAddress, 0x40)
        if not peb_data:
            return {}
        is64 = sys.maxsize > 2**32
        pp_off = 0x20 if is64 else 0x10
        pp = int.from_bytes(
            peb_data[pp_off:pp_off + (8 if is64 else 4)], 'little')
        if pp <= 0x10000:
            return {}
        pp_data = _win_read_mem(handle, pp, 0x100)
        if not pp_data:
            return {}

        def unicode_string(base):
            length = _win_u16(pp_data, base)
            if length <= 0 or length > 65534:
                return ''
            ptr = int.from_bytes(
                pp_data[base + (8 if is64 else 4):
                        base + (16 if is64 else 8)], 'little')
            if ptr <= 0x10000:
                return ''
            raw = _win_read_mem(handle, ptr, length + 2)
            if not raw:
                return ''
            return raw.decode('utf-16-le', 'replace').rstrip('\x00')

        info = {
            'exe': unicode_string(0x60 if is64 else 0x38),
            'cmd': unicode_string(0x70 if is64 else 0x40),
            'cwd': unicode_string(0x38 if is64 else 0x24),
        }
        _WIN_PEB_CACHE['items'][pid] = info
        _WIN_PEB_CACHE['mono'] = time.monotonic()
        return info
    finally:
...
def _win_process_ws(pid):
    """进程工作集（字节）：GetProcessMemoryInfo(ProcessVmCounters)。
    NtQuerySystemInformation 的 WorkingSetSize 在部分 Windows 11 构建上
    不可靠，改用每个进程的 VM 计数器，与任务管理器口径一致。"""
    kernel32 = _win_api()[1]
    import ctypes
    from ctypes import wintypes
    ...
    try:
        ...
        if not psapi.GetProcessMemoryInfo(
            ...
            return 0
        return counters.WorkingSetSize or 0
    finally:
        kernel32.CloseHandle(handle)


def _win_current_user_sid():
    """当前用户的 SID 字符串（S-1-5-21-...）。"""
    global _WIN_SELF_SID
    if _WIN_SELF_SID is None:
        _WIN_SELF_SID = _win_process_sid(os.getpid())
    return _WIN_SELF_SID


def _win_process_sid(pid):
    """进程所属用户的 SID 字符串；读不到返回 None。

    走 OpenProcessToken + GetTokenInformation(TokenUser)，不需要 tasklist，
    也不受提权差异影响（同用户进程一律可读）。"""
    kernel32 = _win_api()[1]
    import ctypes
    from ctypes import wintypes
    ...
    try:
        ...
        buf = ctypes.create_string_buffer(int(size.value or 0) or 64)
        if not advapi32.GetTokenInformation(
                token, 1, buf, len(buf), ctypes.byref(size)):
            return None
        # TOKEN_USER = { SID_AND_ATTRIBUTES { PSID User; DWORD Attributes } }
        sid_ptr = int.from_bytes(buf.raw[:8], 'little')
        if not sid_ptr:
            return None
        sid_string = ctypes.c_wchar_p()
        if not advapi32.ConvertSidToStringSidW(
                ctypes.c_void_p(sid_ptr), ctypes.byref(sid_string)):
            return None
...
def _win_raw_snapshot():
    """合并 NtQuery + 进程 SID + 用户 PEB 信息的全量快照（带 TTL）。"""
    with _WIN_SNAPSHOT_LOCK:
        now = time.monotonic()
        cached = _WIN_SNAPSHOT_CACHE.get('raw')
        if cached is not None and now - _WIN_SNAPSHOT_CACHE.get(
                'mono', 0.0) < _WIN_SNAPSHOT_TTL:
            return cached
        base = _win_nt_query_processes()
        self_sid = _win_current_user_sid()
        raw = {}
        for pid, entry in base.items():
            if self_sid:
                sid = _win_process_sid(pid)
                uid = 0 if sid == self_sid else (sid or -1)
            else:
                uid = -1
            raw[pid] = {
                'ppid': entry['ppid'],
                ...
                'ticks': entry['ticks'],
                'ws': _win_process_ws(pid),
                'uid': uid,
                'exe': '',
                'cmd': '',
            }
        _WIN_SNAPSHOT_CACHE['raw'] = raw
        _WIN_SNAPSHOT_CACHE['mono'] = time.monotonic()
        return raw


def _win_invalidate_snapshot():
    """进程身份写入后强制下一次快照重建，避免 TTL 缓存漏掉刚启动的进程。"""
    with _WIN_SNAPSHOT_LOCK:
        _WIN_SNAPSHOT_CACHE["raw"] = None


def _win_fill_details(pids):
    """为指定 pid 补 PEB 详情（exe/cmd），写回快照缓存。"""
    raw = _WIN_SNAPSHOT_CACHE.get('raw') or {}
    for pid in pids:
        entry = raw.get(pid)
        if entry is None or entry.get('exe'):
            continue
        info = _win_process_peb(pid)
        if info:
            entry['exe'] = info.get('exe') or entry.get('name', '')
            entry['cmd'] = info.get('cmd') or ''


def _win_pid_alive(pid):
    """Windows 版存活探测：OpenProcess + 退出码轮询。"""
    kernel32 = _win_api()[1]
    import ctypes
    from ctypes import wintypes
    ...
    handle = kernel32.OpenProcess(0x1000, False, int(pid))
    if not handle:
        return ctypes.get_last_error() == 5  # 拒绝访问 = 进程存在
    try:
...
def _win_scan_listeners():
    """netstat -ano 监听快照 → {(pid, port): {bind_host, ...}}。"""
...
def _win_cpu_percent(pid, ticks):
    """按两次快照的 CPU 计时差估算占用百分比（可与 ps %cpu 一样超 100）。
    CPU 计时按进程单独记录采样时刻：Windows 计时粒度约 15.6ms，
    若用全局跨度，同一批快照内先后采样的跨度趋近 0，活跃进程也会显示 0%。"""
    now = time.monotonic()
    cache = _WIN_CPU_DELTA
    prev = cache.get('ticks', {}).get(pid)
    prev_t = cache.get('times', {}).get(pid)
    cache['ticks'][pid] = ticks
    cache['times'][pid] = now
    ...
    if span <= 0:
        return 0.0
    return max(0.0, (ticks - prev) / span * 100.0)


def _win_ps_snapshot(pids=None, with_uid=True):
    """Windows 进程快照 → {pid: {uid, comm, args, cpu, mem, etime}}。"""
    raw = _win_raw_snapshot()
    if pids is None:
        wanted = sorted(raw)
    else:
        ...
        if not wanted:
            return {}
    _win_fill_details(wanted)
    total_mem = _win_total_memory()
    now = time.time()
    snap = {}
    for pid in wanted:
        entry = raw.get(pid)
        ...
        mem_pct = (entry['ws'] / total_mem * 100.0) if total_mem else 0.0
        snap[pid] = {
            'uid': entry.get('uid', -1),
            'comm': comm,
            'args': cmd or comm,
            'cpu': _win_cpu_percent(pid, entry['ticks']),
            'mem': round(mem_pct, 2),
            'etime': max(0.0, now - entry['created']),
        }
    return snap


def _win_lsof_cwds(pids):
    """Windows 工作目录：PEB CurrentDirectory（尽力而为）。"""
...
def _win_descendants_map():
    """进程树 → {root: [全部后代 pid, ...]}（等价于 macOS 的 pgid 成员表）。"""
    raw = _win_raw_snapshot()
    children = {}
    ...
    for root in raw:
        ...
        if len(members) > 1 or root in children:
            groups[root] = members
    return groups


def _win_origin_table():
    """{pid: (ppid, args)} 进程来源表。"""
    raw = _win_raw_snapshot()
    _win_fill_details(sorted(raw))
    table = {}
    for pid, entry in raw.items():
        table[pid] = (entry.get('ppid', 0), entry.get('cmd') or '')
    return table


def _win_terminate(pid):
    """TerminateProcess 单个进程。返回 (ok, error)。"""
    kernel32 = _win_api()[1]
    import ctypes
    from ctypes import wintypes
    ...
    if not handle:
        ...
        if not _win_pid_alive(pid):
            return True, None  # 已退出：幂等成功
        if err == 5:
            return False, '没有权限结束该进程'
        return False, '结束进程失败: %d' % err
    try:
...
def _win_taskkill(pid):
    """结束整棵进程树（等价 taskkill /T /F）：先终止根，再逐个终止后代。
    已退出视为成功。"""
    pid = int(pid)
    if not _win_pid_alive(pid):
        return True, None
    # 用新鲜进程表计算后代，避免 TTL 缓存漏掉刚启动的子进程。
    base = _win_nt_query_processes()
    children = {}
    for member_pid, entry in base.items():
        ppid = entry.get('ppid', 0)
        if ppid and ppid != member_pid:
            children.setdefault(ppid, []).append(member_pid)
    members = [pid]
    ...
    while queue:
        ...
        queue.extend(children.get(member, []))
    ok, error = _win_terminate(pid)
    for member in reversed(members):
        if member == pid:
            continue
        if not _win_pid_alive(member):
            continue
        child_ok, child_err = _win_terminate(member)
        if not child_ok and ok:
            ok, error = child_ok, child_err
    return ok, error


def _win_owner_of(pid):
    """单个进程的所属用户 SID；不存在返回 None。
    same_user() 按 SID 精确匹配当前用户。"""
    if not _win_pid_alive(pid):
        return None
    return _win_process_sid(pid)


def _win_launch_env(token, environ=None):
    """Windows 无终端启动环境：保留现有 PATH 并补常见工具目录。"""
    ...
    preferred = []
    if appdata:
        preferred.append(os.path.join(appdata, 'npm'))
        preferred.append(os.path.join(appdata, 'pnpm'))
        preferred.append(os.path.join(appdata, 'yarn', 'bin'))
    if localappdata:
        preferred.append(os.path.join(localappdata, 'Microsoft', 'WinGet', 'Links'))
        preferred.append(os.path.join(localappdata, 'Programs', 'Python', 'Launcher'))
    preferred.append(os.path.join(home, 'scoop', 'shims'))
    preferred.append(os.path.join(home, '.local', 'bin'))
    ...
    env.setdefault('PYTHONUNBUFFERED', '1')  # 重定向到日志文件时仍实时写入
    env[RUN_TOKEN_ENV] = token
    return env


def _win_run_app(marker, command):
    """Windows 启动包装进程：argv 持有随机标记，等待用户命令结束后同码退出。

    简单调用直接 CreateProcess（绕开 cmd /c 引号破坏）；含管道/重定向/
    变量展开等 cmd 语义的命令才交给 cmd /c 执行。"""
    cmd_text = command.strip()
    simple = bool(cmd_text) and not any(
        char in cmd_text for char in '&|<>^%')
    tokens = win_split_command(cmd_text) if simple else None
    # 注意：内层进程不能用 CREATE_NO_WINDOW——该标志会破坏标准句柄继承，
    # 导致子进程输出无法写入日志文件。无控制台时子进程本就不会弹窗。
    try:
        if tokens and tokens[0]:
            try:
                proc = subprocess.Popen(tokens)
            except OSError:
                # Windows 的 npm/pnpm/yarn/npx 等是 .cmd 壳脚本，CreateProcess
                # 无法直接执行；回退到 cmd /c 按原命令执行（退出码仍然透传）。
        ...
        print('启动命令失败: %s' % e, file=sys.stderr)
        return 127
    return proc.wait()

def _win_pick(what):
    """Windows 原生目录/文件选择框（PowerShell + WinForms，系统自带）。
    返回 (path|None, canceled)。"""
    dialog = 'FolderBrowserDialog' if what == 'dir' else 'OpenFileDialog'
    title = '选择工作目录' if what == 'dir' else '选择批处理脚本'
    if what == 'dir':
        # FolderBrowserDialog 没有 Title 属性，只有 Description
        extra = '$d.Description = ' + repr(title) + '; '
        extra += '$d.ShowNewFolderButton = $false; '
        result_expr = '{ $d.SelectedPath }'
    else:
        extra = '$d.Title = ' + repr(title) + '; '
        extra += "$d.Filter = '脚本文件|*.py;*.bat;*.cmd;*.ps1;*.vbs;*.sh;*.exe|所有文件|*.*'; "
        result_expr = '{ $d.FileName }'
    body = (
    ...
    try:
        r = subprocess.run(
            ['powershell.exe', '-NoProfile', '-NonInteractive', '-STA',
             '-ExecutionPolicy', 'Bypass', '-Command', body],
            capture_output=True, text=True, errors='replace', timeout=180,
            creationflags=CREATE_NO_WINDOW)
    except Exception as e:
        LOG.warning('打开系统选择框失败: %s', e)
        return None, False
    if r.returncode != 0:
        # PowerShell 启动/执行失败 ≠ 用户取消；失败要能让前端提示
        LOG.warning('系统选择框执行失败: %s', (r.stderr or '').strip()[:200])
        return None, False
    value = (r.stdout or '').strip()
    if not value:
        return None, True  # 用户取消：静默
    return value.rstrip('\\') or None, False


def _win_msgbox(message, title='总控台', buttons=0, icon=64):
    """Windows 消息框（PowerShell + Microsoft.VisualBasic，系统自带）。
    返回按钮值：OK=1 / Cancel=2 / Yes=6 / No=7。"""
    safe_message = message.replace("'", "''")
    safe_title = title.replace("'", "''")
    body = (
        'Add-Type -AssemblyName Microsoft.VisualBasic; '
        "[Microsoft.VisualBasic.Interaction]::MsgBox('" + safe_message +
        "', " + str(buttons + icon) + ", '" + safe_title + "')")
    try:
        r = subprocess.run(
            ['powershell.exe', '-NoProfile', '-NonInteractive', '-STA',
             '-ExecutionPolicy', 'Bypass', '-Command', body],
...
def scan_listeners():
    ...
    macOS 用 lsof；Windows 用 netstat -ano。
    """
    if IS_WINDOWS:
        return _win_scan_listeners()
...
def ps_snapshot(pids=None, with_uid=True):
    ...
    Windows 走 NtQuerySystemInformation + tasklist + PEB 的组合实现。
    """
    if IS_WINDOWS:
        return _win_ps_snapshot(pids, with_uid)
    base = ["ps"]
    if pids is None:
        base.append("-ax")
    else:
...
def lsof_cwds(pids):
    """进程工作目录 → {pid: cwd}。macOS 用 lsof；Windows 读 PEB。"""
    if IS_WINDOWS:
        return _win_lsof_cwds(pids)
    pids = [int(p) for p in pids]
    if not pids:
        return {}
...
if IS_WINDOWS:
    _WINDOWS_DIR = (os.environ.get("WINDIR") or "C:\\Windows") + "\\"
    SYSTEM_PATH_PREFIXES = (_WINDOWS_DIR, "C:\\Program Files\\WindowsApps\\")
else:
...
def project_name(cwd):
    ...
    if not cwd or cwd == "/" or cwd == HOME_DIR:
        return None
    if re.match(r"^[A-Za-z]:$", cwd):  # Windows 盘符根目录
        return None
...
_ORIGIN_SKIP_NAMES = {
    ...
    "conhost", "conhost.exe", "cscript", "wscript", "rundll32",
...
def origin_snapshot():
    """{pid: (ppid, args)}，供来源溯源。macOS 用 ps；Windows 复用快照。"""
    if IS_WINDOWS:
        return _win_origin_table()
    table = {}
...
def attribute_origin(pid, table):
    ...
    for _ in range(12):
        ...
        if ppid <= 1 or (IS_WINDOWS and ppid == 4):
        ...
        if IS_WINDOWS:
            first = parent_args.split()[0] if parent_args.split() else ""
            first = first.strip('"').lstrip("-")
            exe_base = os.path.basename(first).lower()
            if exe_base in _ORIGIN_EXE_ALIASES:
                label, icon = _ORIGIN_EXE_ALIASES[exe_base]
                return {"label": label, "icon": icon}
...
def build_services(cfg, groups=None):
    ...
    mine_pids = [pid for pid, _ in listeners
                 ...
                 and same_user(snap[pid].get("uid"))]
    ...
    for pid, port in sorted(listeners, key=lambda x: (x[1], x[0])):
        if pid == SELF_PID:
            continue
        info = snap.get(pid)
        if not info or not same_user(info.get("uid")):
            continue
...
def build_watched(keywords):
    ...
    for pid, info in sorted(snap.items()):
        if pid == SELF_PID or not same_user(info.get("uid")):
            continue
        name = os.path.basename(info.get("comm") or "") or "?"
        if name in ("ps", "lsof"):
            continue
        if IS_WINDOWS and name.lower() in ("tasklist", "tasklist.exe",
                                           "netstat", "netstat.exe",
                                           "conhost", "conhost.exe"):
            continue
...
def pgid_members_map():
    """进程组/进程树成员表 → {pgid: [pid, ...]}。
    macOS：ps -axo pid=,pgid=，进程退出后其子孙仍保留原 pgid（被 launchd
    收养也不变），因此按 pgid 能找到「脚本把服务放后台后自己退出」的存活成员。
    Windows：按 ppid 计算进程树后代，等价语义。"""
    if IS_WINDOWS:
        return _win_descendants_map()
    groups = {}
...
def managed_process_index(apps, groups=None):
    ...
    for app in apps:
        ...
        current_user = sorted(
            ...
            if same_user(snap.get(pid, {}).get("uid")))
...
def legacy_managed_pid(app, listeners=None, snap=None, cwds=None):
    ...
    for pid in sorted(port_pids):
        if not same_user(snap.get(pid, {}).get("uid")):
            continue
    ...
    for app in apps_cfg:
        ...
        if owner_pid:
            ...
            port_owner = {
                ...
                "currentUser": same_user(owner_info.get("uid")),
...
def build_health(cfg):
    ...
    for label, path in (("data", DATA_DIR), ("icons", ICONS_DIR),
        ...
        elif not IS_WINDOWS:  # Windows 无 POSIX 权限位，模式检查无意义
            try:
...
def process_uid(pid):
    ...
    Windows 返回 0（当前用户）/-1（其他用户），由 same_user 统一判断。"""
    if IS_WINDOWS:
        if not pid_alive(pid):
            return None
        owner = _win_owner_of(pid)
        if owner is None:
            return None
        return 0 if same_user(owner) else -1
...
def kill_process(pid, force):
    ...
    if uid is None:
        return False, "进程不存在"
    if not same_user(uid):
        return False, "只能结束当前用户的进程"
    if IS_WINDOWS:
        # Windows 无 POSIX 信号：taskkill 结束整棵进程树。
        return _win_taskkill(pid)
...
def stop_pid_tree(pid, sig=signal.SIGTERM):
    ...
    Windows 用 taskkill /T /F 结束整棵进程树。
    """
    if IS_WINDOWS:
        return _win_taskkill(pid)
    try:
...
def build_launch_env(token, environ=None):
    ...
    macOS：Finder/LSUIElement 启动的应用通常只有系统 PATH，不会读取用户
    shell 配置，因此显式补入 Homebrew、npm/pnpm、Volta、NVM、fnm 等目录。
    Windows：GUI 启动继承系统环境变量，补入 npm/pnpm/WinGet/scoop 等目录。
    """
    if IS_WINDOWS:
        return _win_launch_env(token, environ)
    env = dict(os.environ if environ is None else environ)
    home = os.path.expanduser("~")
    preferred = [
...
def start_app(app):
    ...
    # 外层进程在 argv 中持有随机标记并等待内层：macOS 用 bash 双层脚本，
    # Windows 用 server.py --run-app 包装进程。进程树既可验证，
    # 也不会因启动脚本过早退出而失去锚点。
    ...
    try:
        ...
        if IS_WINDOWS:
            proc = subprocess.Popen(
                [sys.executable, os.path.abspath(__file__), "--run-app",
                 marker, app["command"]],
...
def pick_path(what):
    ...
    macOS 用 osascript；Windows 用 PowerShell + WinForms（系统自带）。
    """
    if IS_WINDOWS:
        return _win_pick(what)
    if what == "dir":
...
SHELL_BUILTINS = {
    ...
    "call", "cls", "copy", "del", "dir", "for", "goto", "if", "md", "mkdir",
    "move", "path", "pause", "popd", "prompt", "pushd", "rd", "rem", "ren",
    "rmdir", "start", "time", "title", "ver", "vol", "where", "chcp", "date",
}
...
def _script_target(tokens, cwd):
    ...
    if re.fullmatch(r"python(?:\.exe)?(?:\d+(?:\.\d+)*)?", base, re.I):
...
def inspect_app_health(app):
    ...
    if executable and not direct and executable_base not in SHELL_BUILTINS:
        if "/" in executable or "\\" in executable or re.match(
                r"^[A-Za-z]:", executable):
            runtime = _resolve_command_path(executable, cwd)
            runtime_ok = os.path.isfile(runtime) and (
                os.access(runtime, os.X_OK) if not IS_WINDOWS else True)
        else:
            runtime = executable
            search = executable
            if IS_WINDOWS and executable.lower() == "python3":
                search = "python"  # Windows 常见安装只有 python.exe
            runtime_ok = bool(shutil.which(
                search, path=build_launch_env("health-check").get("PATH")))
        if not runtime_ok:
            add(
...
def detect_project(root):
    ...
    if os.path.isfile(os.path.join(root, "manage.py")):
        ...
        for module_file in ("app.py", "main.py", "server.py"):
            ...
            if "streamlit" in py_deps or imports_streamlit:
                ...
                prefix = "uv run" if python_runner == "uv run" else python_runner
    ...
    mac_scripts = ("start.command", "dev.command", "run.command",
                   "start.sh", "dev.sh", "run.sh")
    win_scripts = ("start.bat", "start.cmd", "dev.bat", "run.bat",
                   "start.ps1", "dev.ps1", "run.ps1")
    for script_name in (win_scripts if IS_WINDOWS else mac_scripts):
        if os.path.isfile(os.path.join(root, script_name)):
            ...
            if IS_WINDOWS:
                runner = ("powershell -NoProfile -ExecutionPolicy Bypass -File"
                          if script_name.endswith(".ps1") else "cmd /c")
                add("%s %s" % (runner, win_quote(script_name)),
...
def stop_target_alive(target, expected_uid=None):
    if target["kind"] == "group":
        if IS_WINDOWS:
            # taskkill /T /F 已发出；整棵树以根进程是否存活为准。
...
def sniff_icon_bytes(data, ctype=""):
    """→ "png" / "jpg" / "webp" / "ico" / "svg" / None。
    只接受以 XML 声明或 <svg 开头的真实 SVG（拒绝把任意 HTML 当图标）；
    SVG 作为 <img> 展示不会执行脚本，与 AGENTS.md 的 favicon 契约一致。"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_fetch_favicon(self, app_id):
        ...
        if not port and configured_port:
            # 配置端口被任意进程监听时也允许抓取（仅回环地址、只读图标），
            # 便于外部运行的服务也能被“识别抓取”站点图标。
            if any(p == configured_port for _, p in listeners):
                port = configured_port
        if not port:
...
def _launcher_dialog(message):
    """询问已运行的实例怎么处理；返回 "打开控制台" / "重新启动" / None。"""
    if IS_WINDOWS:
        # YesNoCancel：Yes=打开控制台（默认）、No=重新启动、Cancel=取消
        value = _win_msgbox(message, buttons=3, icon=64)
        if value == 6:
            return "打开控制台"
        if value == 7:
            return "重新启动"
...
def _launcher_alert(message):
    if IS_WINDOWS:
        _win_msgbox(message, buttons=0, icon=16)
        return
    script = """on run argv
...
def schedule_console_restart(server, preferred_port):
    ...
    helper = subprocess.Popen(
        ...
        cwd=BASE_DIR, close_fds=True, **kwargs)
...
def redirect_console_output():
    ...
    for stream in (sys.stdout, sys.stderr):
        try:
            # Windows 文本层默认会把 \\n 转成 \\r\\n（且 dup2 后可能叠加），
            # 日志文件统一写裸 \\n，与 macOS 行为一致。
            stream.reconfigure(line_buffering=True, newline="")
        except (AttributeError, OSError):
            pass
    # pythonw 无控制台时 sys.stdout/stderr 原本为 None（或已绑定 devnull），
    # 重建为直接绑定 fd 1/2 的对象，让后续输出真正进入 console.log。
...
def _ensure_stdio():
    """pythonw 等无控制台启动时 sys.stdout/stderr 为 None，
    print/日志写入会直接崩溃；先绑定到 devnull 兜底。
    启动器路径随后会用 redirect_console_output 重建为日志文件句柄。"""
...
if __name__ == "__main__":
    if "--prepare-storage" in sys.argv:
        ...
        launcher_main()
    elif "--run-app" in sys.argv:
        # Windows 启动包装：外层进程在 argv 中持有随机标记并等待用户命令。
        index = sys.argv.index("--run-app")
        try:
            marker = sys.argv[index + 1]
            command = sys.argv[index + 2]
        except IndexError:
            sys.exit(2)
        sys.exit(_win_run_app(marker, command))
